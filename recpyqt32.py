...
class Canvas(QtWidgets.QWidget):
    ...
    def paintEvent(self, event):
        ...
        for index, rect in enumerate(self.rect_list):
            # Create a 50% masked rectangle
            path = QtGui.QPainterPath()
            path.addRect(rect)
            path.addRect(rect.center().x() - rect.width() / 4, rect.center().y() - rect.height() / 4, rect.width() / 2, rect.height() / 2)
            path.setFillRule(Qt.WindingFill)

            # Set the clipping path to the 50% masked rectangle
            qp.setClipPath(path.toFillPolygon().toPolygon(), QtGui.QRegion.Inverse)
        ...
        qp.end()  # Release the QPainter object when done

        self.backingStore.endPaint()  # Call QBackingStore::endPaint() after releasing the QPainter object
