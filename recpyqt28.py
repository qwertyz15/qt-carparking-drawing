...
class Canvas(QtWidgets.QWidget):
    ...
    def save(self):
        ...
        if filename:
            ...
            # self.rect_list_toupled = [ ((rect.x() * self.x_scale, rect.y() * self.y_scale, rect.width() * self.x_scale, rect.height() * self.y_scale), occ) for rect, occ in self.rect_occu_list]
...
class RectangleInfoWidget(QtWidgets.QWidget):
    ...
    def add_rectangle(self, tup):
        rect, occupancy = tup
        sno = self.rect_list_widget.count() + 1
        occu_text = ("Occupied" if occupancy else "Unoccupied")
        item_text = f"Slot {sno}: {occu_text}"
...
class MainWidget(QtWidgets.QWidget):
    ...
    def on_rectangle_added(self, tup):
        self.rectangle_info_widget.add_rectangle(tup)
